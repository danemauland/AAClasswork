...
def teachers_and_divisions
  ...
  execute(<<-SQL)
  select teachers.name, (CASE when dept_id = 1 or dept_id = 2 then 'Sci' else 'Art' end)
  from teachers
  SQL
end
...
def teachers_and_divisions_two
  ...
  # 'None' otherwise.
  execute(<<-SQL)
  select teachers.name, (CASE when dept_id = 1 or dept_id = 2 then 'Sci' WHEN dept_id = 3 then 'Art' else 'None' end)
  from teachers
  SQL
end
